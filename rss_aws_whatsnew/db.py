...
class FeedEntryDB:
    ...
    def __init__(self, db_name):
        ...
        # self.init_db()
        # "OpenSearch Service", "OpenSearch Service - Ingestion", "OpenSearch Service - Serverless", "OpenSearch Service - Managed Cluster" -> "OpenSearch"
        # "Managed Service for Prometheus", Grafana, Kafka -> Prometheus, Grafana, Kafka
        # Kinesis Analytics", "Kinesis Firehose", "Kinesis Streams" -> "Kinesis"
        self.target_services = [
            "Kinesis", "OpenSearch Service", "QuickSight", "Redshift", "Kendra",
    ...
    def save_entries(self, feed):
        ...
        for entry in feed.entries:
            ...
            if published_parsed:
                published_parsed = datetime.fromtimestamp(time.mktime(published_parsed)).strftime('%Y-%m-%d %H:%M:%S')
            else:
                published_parsed = ''
            values = (
                entry.get('id', ''),
            ...
            try:
                c.execute("INSERT OR REPLACE INTO feed_entries VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", values)
    ...
    def get_recent_entries(self, days=7):
        ...
        c.execute("SELECT * FROM feed_entries WHERE published_parsed >= ?", (recent_date.strftime('%Y-%m-%d %H:%M:%S'),))
        rows = c.fetchall()
        print("rows", len(rows))
        conn.close()
        return rows
    ...
    def delete_old_entries(self, days=30):
        ...
        recent_date = datetime.now() - timedelta(days=days)
        print("recent_date:", recent_date)
        c.execute("DELETE FROM feed_entries WHERE published_parsed < ?", (recent_date.strftime('%Y-%m-%d %H:%M:%S'),))
        conn.commit()

        # get table length after deleting
        c.execute("SELECT COUNT(*) FROM feed_entries")
        table_size_after_delete = c.fetchone()[0]
        print("delete articles num:", table_size_before_delete - table_size_after_delete)
        conn.close()
    
    def contains_service_word(self, text):
        for service_words in self.target_services:
            
            if re.search(r'\b' + re.escape(service_words) + r'\b', text, re.IGNORECASE):
                #re.escapeを使って正規表現のメタ文字をエスケープしています。
                #\bを使って単語の境界を指定することで、サービス名の単語が含まれる場合のみマッチ
