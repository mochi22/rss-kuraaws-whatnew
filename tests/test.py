...
class TestFeedEntryDB(unittest.TestCase):
    ...
    def test_get_or_create_table(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_with(f"Table '{new_table_name}' created successfully.")
    ...
    def test_get_recent_entries(self):
        ...
        self.table.put_item(Item={
            'id': '1',
            'title': 'Old Entry',
            'published_parsed': (now - timedelta(days=8)).strftime('%Y-%m-%d %H:%M:%S')
        })
        self.table.put_item(Item={
            'id': '2',
            'title': 'Recent Entry',
            'published_parsed': (now - timedelta(days=3)).strftime('%Y-%m-%d %H:%M:%S')
        })
    ...
    def test_delete_old_entries(self):
        ...
        self.table.put_item(Item={
            'id': '1',
            'title': 'Old Entry',
            'published_parsed': (now - timedelta(days=40)).strftime('%Y-%m-%d %H:%M:%S')
        })
        self.table.put_item(Item={
            'id': '2',
            'title': 'Recent Entry',
            'published_parsed': (now - timedelta(days=10)).strftime('%Y-%m-%d %H:%M:%S')
        })
